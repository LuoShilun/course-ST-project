# -*- coding: utf-8 -*-
"""pytest全局配置与执行结果记录（模块二 · 方案2「AI测」）。

结构与模块一一致：除常规 fixture 外，本文件实现一个轻量结果记录器，
把每个用例的 nodeid、用例编号、执行状态、实际结果写入 reports/results.json。
"""
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "ai: 由AI生成/辅助生成的用例（模块二·方案2）")
